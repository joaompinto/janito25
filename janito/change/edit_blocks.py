...
def get_edit_blocks(response: str) -> Tuple[List[CodeChange], str]:
    ...
    modified_response = []
    current_block = []
    original_content = None  # Renamed from before_block
    ...
    for line in response.splitlines():
        ...
        if line.startswith("<<<< original") or line.startswith("<<<< starting"):
            ...
            if current_command == "Clean":
                original_content = current_block  # Renamed
            elif not original_content and current_block:  # Renamed
                original_content = current_block  # Renamed
            current_block = []
            in_block = True
            ...
            if current_command == "Delete":
                ...
                edit_blocks.append(CodeChange(
                    filename, reason,
                    trim_block(original_content or []),  # Renamed
                ...
                edit_blocks.append(CodeChange(
                    filename, reason,
                    [],                          # No original content
                    trim_block(current_block),   # Modified content
                    EditType.CREATE,
                    current_marker
                ))
            elif current_command == "Edit":
                original = trim_block(original_content or [])  # Renamed
